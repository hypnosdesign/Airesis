class ProposalsWorker < ApplicationJob
  ...
  def perform(*args)
    ...
    when LEFT24
      ...
      NotificationProposalTimeLeft.perform_later(proposal.id, '24_hours')
    when LEFT1
      # invia una notifica a tutti i partecipanti alla proposta
      # che non hanno dato la loro valutazione o possono cambiarla
      # 1 ora prima della chiusura del dibattito
      NotificationProposalTimeLeft.perform_later(proposal.id, '1_hour')
    when LEFT24VOTE
      # send a notification to all participants that can vote the proposal and haven't voted it yet
      # 24 ore prima della chiusura del dibattito
      NotificationProposalTimeLeftVote.perform_later(proposal.id, '24_hours_vote')
    when LEFT1VOTE
      # send a notification to all participants that can vote the proposal and haven't voted it yet
      # 1 ora prima della chiusura del dibattito
      NotificationProposalTimeLeftVote.perform_later(proposal.id, '1_hour_vote')
    else
