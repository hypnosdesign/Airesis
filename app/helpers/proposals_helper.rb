module ProposalsHelper
  def navigator_actions(args = {})
    ...
    (link_to '#', onclick: 'return false', class: "#{classes} move_up" do
      '<i class="fa-solid fa-arrow-up"></i>'.html_safe
    end) +
      (link_to '#', onclick: 'return false', class: "#{classes} move_down" do
        '<i class="fa-solid fa-arrow-down"></i>'.html_safe
      end) +
  ...
  def proposal_group_image_tag(proposal)
    if @group || !proposal.group
      ...
      image_tag(proposal.group.image.attached? ? url_for(proposal.group.image) : asset_path('gruppo-anonimo.png'), title: proposal.group.name, data: { qtip: '' })
    end
  end
